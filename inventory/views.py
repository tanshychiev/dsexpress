...
def confirm_stock_view(request):
    ...
    - confirm one product row or confirm all products
    - if real qty differs from system available qty, confirm_stock() handles the adjustment
    """
    if not staff_only(request):
        return redirect("portal:dashboard")
    ...
    if request.method == "POST":
        ...
        # Confirm only one row when the row-level Confirm button is clicked.
